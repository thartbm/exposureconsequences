...
In the manuscript we'll first show that there indeed are motor changes after exposure training (with the no-cursor data) and then compare that with the classic. Second is the localization: a) localization shifts after exposure b) the shifts are (not) different from those after classic c) the effects are (not) different across the workspace d) this pattern is (not) different from that after classic. A third topic might be if localization shifts can predict no-cursor changes, which we'll call the "correlation"  topic. Here we'll also follow this order:
...
Since participants trained with a single target, we could look at generalization curves. These appear to be shifted more counter clockwise in most cases, so we want to quantify this. But, for most of the curves, and especially the ones that seems to be shifted, we don't have data on the full curve on one side of the peak. Or maybe it doesn't even include the peak itself. That makes it hard to fit a normal function to the data. Instead, we assume that the minimal change in the data, occurs far beyond the boundaries of the measured workspace, make the data cumulative and scale back to a range from 0 to 1. This allows fitting a _cumulative_ normal function where the 50% point corresponds to the peak of the generalization curve. This method is more stable, and actually works on all the datasets used (but we didn't test online localization from the classic experiment). However, the results should still be considered carefully.

# Reach Aftereffects

Here is a plot, probably Figure 2:

```{r fig2_reachaftereffect, fig.width=7.5, fig.height=2.5}
...
So there is no effect of iteration here, nor does it interact with target, which means the reach aftereffects are the same for the first iteration and the average across the other four iterations. We now also have enough power to see an effect of target angle: the generalization curve is not flat.
...
```{r fig3_localization, fig.width=7.5, fig.height=2.5}
plotLocalization(classicOnline=TRUE)
```
...
kable(getLocCountTable())
```

Since the majority of missing data appears to be at the 15 degree location, we exclude that angle from analysis, with a switch in analysis functions `remove15=TRUE`, which can be set to false if we'd want.

## Is there a localization shift after exposure training?

We first want to see if there is any overall effect of aligned versus rotated training in the exposure group (i.e.: are there any localization shifts at all?), with movement type and hand angle as fixed effects, and particiant as random effect.

```{r loc_exp}
exposureLocalization(remove15=TRUE, LMEmethod=LMEmethod)
```
...
```{r loc_exp_movtype}
exposureLocalizationShift(remove15=TRUE, LMEmethod=LMEmethod)
```
...
```{r loc_ombnibus}
groupLocalization(model='full', remove15=TRUE, LMEmethod=LMEmethod)
```

In the exposure group there is no effect of movement type, but we see one here. This could be because the effect of movement type is much larger in the classic group. This would predict an interaction between group and movement type, but that's not the case (p=.133).

What if we don't care about hand angle:

```{r loc_groups_movementtype}
groupLocalization(model='movementtype', remove15=TRUE, LMEmethod=LMEmethod)
```
...
```{r loc_cla}
classicLocalizationShift(remove15=TRUE, LMEmethod=LMEmethod)
```

So there is a fairly strong effect of movement type in the classic group. Together with the absence of an effect of movement type in the exposure group, but no interaction between group and movement type if we combine both datasets.

Also note the absence of an effect of hand angle in the classic group here -- probably because we excluded the 15 degree target, but it seems that after classic training the generalization curve for delayed localization is much flatter.
...
4. This predicts an interaction between movement type and group in the combined analysis, but we don't see that.
...
These results mimic those of the reach aftereffects: the median values are different, but they each fall in the others confidence interval. However, here the LME showed that there was an interaction between training type and hand angle, which suggests that the generalization curves are different. If we want to look at this better, we need to get better estimates of these curves in every participant, by having a wider range of points and more data per point, but probably also larger rotations.
...
Only one is significant: active localization predicting reach aftereffects in the exposure group.
...
plotLocalization(classicOnline=TRUE, generateSVG=TRUE)
