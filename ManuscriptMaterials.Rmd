...
This is a collection of statistics and figures for the manuscript on the exposure consequences project.
...
_For Panel B: We're not really interested in reach aftereffects per se. One focus of this plot/analyses could be the motor changes evoked by the two different types of training. Then it would make more sense to use the initial rotated no-cursor block (I think this is what it does now), as they are directly evoked by the training, and could reflect the motor changes themselves better. Or do we want to focus on the motor changes that are present throughout localization? In that case it makes more sense to use the four later tasks, as they decay stops fairly quickly, so these might be said to better reflect the motor changes that are present throughout the localization tasks. We could also use some average... I can see it many different ways, just want to have a clear strategy here._
