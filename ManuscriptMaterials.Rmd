...
Alternatively, if you want to thoroughly check the results, you can run this chunk to delete the pre-calculated results (set delete=TRUE and download=FALSE), which will force the code to re-calculate it. By default, the code here will use "only" 1000 iterations for bootstrapping, which will run 20 times faster, but also guarantees that the confidence intervals are less precise than the ones we pre-calculated.
