...
```{r}
participants <- load.DownloadDataframe(informationURLs['demographics'],'participants_exposure.csv')
...
```{r}
print(c('mean'=mean(participants$age), 'sd'=sd(participants$age)))
```

And the distribution of sex:

```{r}
print(table(participants$sex))
```
...
plotLocalization(classicOnline=TRUE,selectPerformance=selectPerformance)
```
...
```{r loc_exp}
exposureLocalization(remove15=TRUE, LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```
...
```{r loc_exp_movtype}
exposureLocalizationShift(remove15=TRUE, LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```
...
```{r loc_ombnibus}
groupLocalization(model='full', remove15=TRUE, LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```
...
groupLocalization(model='movementtype', remove15=TRUE, LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```
...
plotReachAftereffects(selectPerformance=FALSE)
```
...
exposureNoCursorChange(LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```
...
exposureAftereffectsPersistent(LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```

So there is no effect of iteration here, nor does it interact with target, which means the reach aftereffects are the same for the first iteration and the average across the other four iterations. We now also see an effect of target angle: the generalization curve is not flat. This seems mostly due to the extra training.
...
```{r RAE_exp_cla}
exposureClassicReachAftereffects(LMEmethod=LMEmethod, selectPerformance=selectPerformance)
```
...
```{r RAE_peaks}
getPeakConfidenceInterval('classic', part='all', CIs=c(.95), selectPerformance=selectPerformance)
getPeakConfidenceInterval('exposure', part='all', CIs=c(.95), selectPerformance=selectPerformance)
```
...
#plotBlinkDetection(generateSVG=TRUE)
```
