...
2. Exposure training leads to shifts in localization, that are not very different between active and passive localization. Since both active and passive movements access recalibrated proprioception, while active localization also accesses updated predictions, this indicates that hand localization is makes very good use of proprioception.
    ...
    3. Within the workspace however, there is no interaction between movement type and training group (but there is a trend if hand angle is used as a random effect, in all analyses the 15 degree data is omitted as Denise said). Perhaps there is a small update of predicted proprioceptive consequences, that we can't parcel out. A more computational approach would be needed to see if this can explain the pattern of results here.
